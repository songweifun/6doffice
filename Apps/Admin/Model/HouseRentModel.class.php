...
class HouserentModel extends Model{
     ...
     * @return mixed
     */
    function getImgList($houserent_id,$field = '*')
    {
        return M('houserent_pic')->where(array('houserent_id'=>$houserent_id))->select();
    ...
    function deleteRent($ids) {
        ...
        if (is_array($ids)) {
            ...
            $deletewhere1 = 'houserent_id in (' . $ids . ')';
            $deletewhere2 = 'house_id in (' . $ids . ')';
        } else {
            $where = 'id=' . intval($ids);
            $deletewhere = 'id='.$ids ;
            $deletewhere1 = 'houserent_id ='.$ids ;
        ...
        foreach ($broughId as $key=> $item){
            ...
            D('Statistics')->where(array('stat_index'=>'rentNum'))->setDec('stat_value',1);
        }
        ...
        M('houserent_pic')->where($deletewhere1)->delete();//删除图片
        M('houserent_stat')->where($deletewhere2)->delete();//删除房源点击
        M('houserent_bargain')->where($deletewhere2)->delete();//删除成交表*/
        return true;
     ...
     * 图片数量
     *
     * @param string 出售房源ID $houserent_id
     * @return number
     */
    function getImgNum($houserent_id)
    {
        return M('houserent_pic')->where(array('houserent_id'=>$houserent_id))->count();
    }
    ...
    {
        return M('houserent_pic')->where(array('id'=>$picid))->delete();
    }
    /**
     * 删除房源的户型图
     *
