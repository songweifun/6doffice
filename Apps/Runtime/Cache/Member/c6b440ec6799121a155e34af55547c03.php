...
<head>
    ...
    <title><?php echo ($title); ?> - 租房源管理</title>
        ...
        <div class="memberBox">
            <div class="memberBoxTab">
                <ul>
                    ...
                    <li><a href="<?php echo U(MODULE_NAME.'/HouseRent/index');?>" class="linkOn"><span>出租管理</span></a></li>
                </ul>
            </div>
            <div class="manageSub">
                <ul class="manageSubNav">
                    <li class="linkOn"><a href="#"><span>出租中</span></a></li>
                    <li><a href="<?php echo U(MODULE_NAME.'/ManageRent/rentTopDone');?>"><span>置顶中</span></a></li>
            ...
            <div class="manageBox">
                <div class="houseSearch">
                    <form name="searchForm" action="" method="GET">
                        <input type="text" class="input" name="q" size="30" value="<?php if($q != null): echo ($q); else: ?>输入房源编号或小区名称<?php endif; ?>" onblur="if(this.value ==''||this.value == '输入房源编号或小区名称'){this.value = '输入房源编号或小区名称';}" onfocus="if(this.value == '输入房源编号或小区名称'){this.value = '';}" />&nbsp;<input type="button" value="搜 索" onclick="javascript:document.searchForm.submit();" />&nbsp;<a href="#notice">租房源管理必读</a><span class="tip">您已发布<span class="f90"><?php echo ($houseNum); ?></span>条出租房源，<?php if($houseLeft >= 0): ?>还可发布<span class="f90"><?php echo ($houseLeft); ?></span>条出租房源<?php else: ?>已超出<span class="f90"><?php echo (abs($houseLeft)); ?></span>条<?php endif; ?> <?php if($memberVip == 0): ?>| <a href="#" onclick="showBoxnum();return false;">购买条数</a><?php endif; ?></span>
                    </form>
                </div>
                <div class="houseList">
                        ...
                        <form name="myform" enctype="text/html" action="" method="POST">
                            ...
                            <?php if(is_array($dataList)): foreach($dataList as $key=>$item): ?><tr>
                                ...
                                <td> <!--增加预约刷新按钮 hyjfc.com-->
                                    <?php if($item["is_appo"] != false): ?><img src="/6doffice/Public/images/time.gif" width="23" height="16" align="absmiddle"  />
                                    <a href="<?php echo U(MODULE_NAME.'/Appointment/index');?>/action/appoShowHouse/site/rent/house_id/<?php echo ($item["id"]); ?>" title="查看">查看</a>|<a href="<?php echo U(MODULE_NAME.'/Appointment/index');?>/action/appoDel/site/rent/house_id/<?php echo ($item["id"]); ?>" title="取消">取消</a>

                                    <?php else: ?>
                                    <a href="<?php echo U(MODULE_NAME.'/Appointment/index');?>/action/appoRefresh/site/rent/ids/<?php echo ($item["id"]); ?>" title="立即预约">立即预约</a><?php endif; ?></td>
                                <td><a href="<?php echo U(MODULE_NAME.'/HouseRent/addrent');?>/action/edit/id/<?php echo ($item["id"]); ?>" title="编辑房源信息">编辑</a>&nbsp;<a href="?house_id=<?php echo ($item["id"]); ?>" onclick="showBox(<?php echo ($item["id"]); ?>);return false;" title="该房源已成交">成交</a>&nbsp;<a href="?house_id=<?php echo ($item["id"]); ?>" onclick="showBoxtop(<?php echo ($item["id"]); ?>);return false;" title="将此房源置顶">置顶</a>&nbsp;<a href="?house_id=<?php echo ($item["id"]); ?>" onclick="showBoxOwner(<?php echo ($item["id"]); ?>);return false;" title="查看房东信息">房东</a></td>
...
<script language="javascript">
    $('#listOperationBtn1').click(function(){
        ...
        if(confirm('你确认刷新选中的房源么？')){
            myform.action='<?php echo U(MODULE_NAME."/ManageRent/refresh");?>';
            myform.submit();}
    ...
    $('#listOperationBtn2').click(function(){
        ...
        if(confirm('你确认预约刷新选中的房源么？')){
            myform.action='<?php echo U(MODULE_NAME."/Appointment/index");?>/action/appoRefresh/site/rent';
            myform.submit();}
    ...
    $('#listOperationBtn3').click(function(){
        ...
        if(confirm('你确认下架选中的房源么？')){
            myform.action='<?php echo U(MODULE_NAME."/ManageRent/notrent");?>';
            myform.submit();}
    ...
    function showBoxOwner(item_id){
        ...
        var url='<?php echo U(MODULE_NAME."/ManageRent/landlordInfo");?>/house_id/'+item_id
        layer.open({
            type: 2,
            title: ['业主信息', ''],
            maxmin: true,
    ...
    function showBoxtop(item_id){
        var url='<?php echo U(MODULE_NAME."/ManageRent/rentTop");?>/house_id/'+item_id
        layer.open({
            type: 2,
            title: ['房源置顶', ''],
            maxmin: true,
    ...
    function showBoxnum(){
        var url='<?php echo U(MODULE_NAME."/ManageRent/buyRentNum");?>'
        layer.open({
            type: 2,
            title: ['购买条数', ''],
            maxmin: true,
