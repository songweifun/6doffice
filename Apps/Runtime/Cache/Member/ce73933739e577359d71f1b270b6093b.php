...
<head>
    ...
    <title><?php echo ($title); ?>  - 查看垃圾箱</title>
        ...
        <div class="memberBox">
            <div class="msgOperationBox">
                <ul class="msgOperation">
                    ...
                    <li class="linkOn"><a href="<?php echo U(MODULE_NAME.'/ManageMessage/msgGarbage');?>"><span>垃圾箱</span></a></li>
                </ul>
            </div>
            <div class="manageBox">
                <div class="houseList">
                    <table border="0" cellpadding="0" cellspacing="1">
                        ...
                        <form name="myform" action="" method="POST">
                            ...
                            <?php if(is_array($dataList)): foreach($dataList as $key=>$item): ?><tr>
                                ...
                                <td><?php if($item['msg_from'] == $username): ?>我<?php else: echo ($item["msg_from"]); endif; ?></td>
                                <td><?php if($item['msg_to'] == $username): ?>我<?php else: echo ($item["msg_to"]); endif; ?></td>
                                <td><?php echo (date("Y-m-d H:i",$item["add_time"])); ?></td>
                            </tr><?php endforeach; endif; ?>
                            <tr>
                                <td colspan="5" class="listOperation">
                                    <label for="checkBoxAll"><input type="checkbox" id="checkBoxAll" name="checkBoxAll" value="1" onclick="checkAll(this);" />全选</label>
                                    <input class="listOperationBtn" type="button" value="彻底删除选中的信件" onclick="if(confirm('你确认要删除选中的信件么？')){myform.action='?action=addDel';myform.submit();}" />
                                </td>
                            </tr>
                            </tbody>
                        </form>
