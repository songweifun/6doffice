...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
			this.startResizedUpload(this.getFile(0).ID, 640, 480, SWFUpload.RESIZE_ENCODING.JPEG, 100);
		}
	} catch (ex) {
		this.debug(ex);
	}
...
serverData=serverData.split('||');
		if (serverData[0].substring(0, 7) === "FILEID:") {
			...
			new uploadHousePicture( serverData[1],serverData[2],serverData[3]);
