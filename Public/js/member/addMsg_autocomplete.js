    ...
    $( "#msg_to" ).autocomplete({
        ...
        source: function( request, response ) {
            ...
            if(term in cache){ //本地缓存 其实就是用一个数组保存已经发送过请求的key和对应返回的data对象
                ...
                response($.map(data,function(item){
                    return {label:item.realname,value:item.realname,member_id:item.id}
                }))
                ...
                $.ajax({
                    ...
                    url:hosesellajaxurl+"/action/getMemberList",
                    dataType:'jsonp',
                    data:{
                        q:request.term
                    },
        ...
        select: function(event, ui) {
            ...
            if(ui.item.value=='我要创建新小区'){
                ...
                $("#reply_to").val(ui.item.member_id);
        ...
        position: { using:function(pos){
            ...
            $(this).css({'top':'topOffset','width':'230','height':'220','overflow':'scroll'});
